...
# 7 random (snuffle) card taking into game form pack_of_card
  ...
  def card_in_game
    ...
    #getter method for cards prepared for Combinations class
  ...
  def array_suit_rank
    ...
    (0..6).each do |i|
      ...
      s<<(@cards[i][1]) if @cards[i][0] == 4 end
    # sort rank in any suit
    { :suit_H => h, :suit_D => d, :suit_C => c, :suit_S => s }.each do |suit, rank|
      rank.sort! { |first, second| second <=> first } end
  end
...
def find_combination
  s = CardOnTable.new
  g =Combinations.new(s.card_in_game)
  g.winner_comb
  print "\n Give me name of combinations you wonna see on the table.\n You can choose of: "
  $the_win_comb.each{|name,_m| print name, '  '}
